...
import xbar.projection.{NoPhrase, Phrase, V_, V}
...
object VBarType {
  ...
  final case class LowerVBar[X : Phrase : CanComplementV](head: V, complement: Option[X]) extends VBarType {
  ...
  object LowerVBar {
    def apply(head: V): LowerVBar[NoPhrase] = LowerVBar(head, Option.empty[NoPhrase])
  }
