...
object J_ {
  ...
  def apply[X : Phrase : CanComplementJ](head: J, complement: X): J_ = J_(JBT(LowerJBar(head, Some(complement))))
...
object R_ {
  def apply(head: R): R_ = R_(RBT(LowerRBar(head, Option.empty[NoPhrase])))
  def apply[X : Phrase : CanComplementR](head: R, complement: X): R_ = R_(RBT(LowerRBar(head, Some(complement))))
...
object V_ {
  def apply [X : Phrase : CanComplementV](head: V, complement: X): V_ = V_(VBT(LowerVBar(head, Some(complement))))
  def apply(head: V): V_ = V_(VBT(LowerVBar(head, Option.empty[NoPhrase])))
...
object N_ {
  def apply[X : Phrase : CanComplementN](head: N, complement: X): N_ = N_(NBT(LowerNBar(head, Some(complement))))
  def apply(head: N): N_ = N_(NBT(LowerNBar(head, Option.empty[NoPhrase])))
